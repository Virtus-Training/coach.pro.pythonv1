...
class NutritionController:
    ...
    def generate_automatic_meal_plan(
        self, client_id: int, nom_plan: str = None, duree_jours: int = 1
    ) -> PlanAlimentaireDTO:
        ...
        try:
            ...
            print(
                f"Plan généré: {plan_simple['nom']} avec {len(plan_simple['repas'])} repas"
            )
    ...
    def create_or_update_nutritional_profile(self, client_id: int, profile_data: dict):
        ...
        print(
            f"Profil nutritionnel simulé créé pour client {client_id}: {profile_data}"
        )
