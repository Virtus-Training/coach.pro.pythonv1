...
class FormCollectif(Card):
    ...
    def __init__(
        ...
        header_frame = ctk.CTkFrame(self, fg_color=("gray95", "gray20"), corner_radius=12)
        ...
        main_params_frame = ctk.CTkFrame(self, fg_color=("gray98", "gray17"), corner_radius=8)
        main_params_frame.grid(row=1, column=0, sticky="ew", padx=16, pady=(0, 12))
        main_params_frame.grid_columnconfigure((0, 1), weight=1)

        # Type de séance
        ctk.CTkLabel(main_params_frame, text="🏋️ Type de séance", font=ctk.CTkFont(weight="bold")).grid(
            row=0, column=0, sticky="w", padx=16, pady=(12, 4)
        )
        self.course_type_var = ctk.StringVar(value="Crossfit")
        course_menu = ctk.CTkOptionMenu(
            main_params_frame, variable=self.course_type_var, values=COURSE_TYPES_DISPLAY,
            fg_color=("gray90", "gray25"), button_color=("gray80", "gray30")
        )
        course_menu.grid(row=1, column=0, sticky="ew", padx=16, pady=(0, 12))

        # Durée avec indicateur visuel
        ctk.CTkLabel(main_params_frame, text="⏱️ Durée", font=ctk.CTkFont(weight="bold")).grid(
            row=0, column=1, sticky="w", padx=16, pady=(12, 4)
        )
        self.duration_var = ctk.StringVar(value="45")
        duration_menu = ctk.CTkOptionMenu(
            main_params_frame, variable=self.duration_var, values=DURATIONS,
            fg_color=("gray90", "gray25"), button_color=("gray80", "gray30")
        )
        duration_menu.grid(row=1, column=1, sticky="ew", padx=16, pady=(0, 12))

        # Section Intensité avec design moderne
        intensity_section = ctk.CTkFrame(self, fg_color=("gray98", "gray17"), corner_radius=8)
        ...
        intensity_menu = ctk.CTkOptionMenu(
            intensity_controls, variable=self.intensity_var, values=intensity_options,
        ...
        equip_section = ctk.CTkFrame(self, fg_color=("gray98", "gray17"), corner_radius=8)
        ...
        ctk.CTkFrame(equip_section, height=12, fg_color="transparent").grid(row=2, column=0)

        # Section Variabilité avec design moderne
        variability_section = ctk.CTkFrame(self, fg_color=("gray98", "gray17"), corner_radius=8)
        ...
        var_header.grid(row=0, column=0, columnspan=2, sticky="ew", padx=16, pady=(12, 8))
        ...
        var_controls.grid(row=1, column=0, columnspan=2, sticky="ew", padx=16, pady=(0, 12))
        var_controls.grid_columnconfigure(1, weight=1)

        self.variability_var = ctk.DoubleVar(value=50)
        self.variability_value_label = ctk.CTkLabel(var_controls, text="50%", font=ctk.CTkFont(weight="bold"))
        self.variability_value_label.grid(row=0, column=0, sticky="w", padx=(0, 12))

        variability_slider = ctk.CTkSlider(
            var_controls, from_=0, to=100, variable=self.variability_var,
        ...
        volume_section = ctk.CTkFrame(self, fg_color=("gray98", "gray17"), corner_radius=8)
        ...
        vol_header.grid(row=0, column=0, columnspan=2, sticky="ew", padx=16, pady=(12, 8))
        ...
        vol_controls.grid(row=1, column=0, columnspan=2, sticky="ew", padx=16, pady=(0, 12))
        vol_controls.grid_columnconfigure(1, weight=1)

        self.volume_var = ctk.DoubleVar(value=50)
        self.volume_value_label = ctk.CTkLabel(vol_controls, text="50%", font=ctk.CTkFont(weight="bold"))
        self.volume_value_label.grid(row=0, column=0, sticky="w", padx=(0, 12))

        volume_slider = ctk.CTkSlider(
            vol_controls, from_=0, to=100, variable=self.volume_var,
        ...
        continuum_section = ctk.CTkFrame(self, fg_color=("gray98", "gray17"), corner_radius=8)
        ...
        cont_header.grid(row=0, column=0, columnspan=2, sticky="ew", padx=16, pady=(12, 8))
        CardTitle(cont_header, text="⚡ Continuum Cardio → Renforcement").pack(side="left")

        # Contrôle avec valeur affichée
        cont_controls = ctk.CTkFrame(continuum_section, fg_color="transparent")
        cont_controls.grid(row=1, column=0, columnspan=2, sticky="ew", padx=16, pady=(0, 12))
        cont_controls.grid_columnconfigure(1, weight=1)

        self.continuum_var = ctk.DoubleVar(value=0)
        self.continuum_value_label = ctk.CTkLabel(cont_controls, text="Équilibré", font=ctk.CTkFont(weight="bold"))
        self.continuum_value_label.grid(row=0, column=0, sticky="w", padx=(0, 12))

        continuum_slider = ctk.CTkSlider(
            cont_controls, from_=-100, to=100, variable=self.continuum_var,
            command=self._update_continuum_display
        )
        continuum_slider.grid(row=0, column=1, sticky="ew")

        # Section Focus avec design moderne
        focus_section = ctk.CTkFrame(self, fg_color=("gray98", "gray17"), corner_radius=8)
        ...
        focus_header.grid(row=0, column=0, columnspan=2, sticky="ew", padx=16, pady=(12, 8))
        ...
        focus_controls.grid(row=1, column=0, columnspan=2, sticky="ew", padx=16, pady=(0, 12))
        focus_controls.grid_columnconfigure(0, weight=1)

        self.focus_var = ctk.StringVar(value="Full-body")
        focus_menu = ctk.CTkOptionMenu(
            focus_controls, variable=self.focus_var, values=FOCUS_OPTIONS,
            font=ctk.CTkFont(weight="bold")
        )
        focus_menu.grid(row=0, column=0, sticky="ew")

        # Section Objectif avec design moderne
        objective_section = ctk.CTkFrame(self, fg_color=("gray98", "gray17"), corner_radius=8)
        ...
        obj_header.grid(row=0, column=0, columnspan=2, sticky="ew", padx=16, pady=(12, 8))
        ...
        obj_controls.grid(row=1, column=0, columnspan=2, sticky="ew", padx=16, pady=(0, 12))
        obj_controls.grid_columnconfigure(0, weight=1)

        self.objective_var = ctk.StringVar(value="Force")
        objective_menu = ctk.CTkOptionMenu(
            obj_controls, variable=self.objective_var, values=OBJECTIVE_OPTIONS,
            font=ctk.CTkFont(weight="bold")
        )
        objective_menu.grid(row=0, column=0, sticky="ew")

        # Section Auto-inclusion avec design moderne
        auto_section = ctk.CTkFrame(self, fg_color=("gray98", "gray17"), corner_radius=8)
        ...
        for idx, opt in enumerate(AUTO_INCLUDE_OPTIONS):
            ...
            checkbox = ctk.CTkCheckBox(
                auto_grid, text=opt, variable=var,
                font=ctk.CTkFont(size=12)
            )
        ...
        toggle_frame = ctk.CTkFrame(custom_section, fg_color=("gray90", "gray20"), corner_radius=8)
        ...
        count_frame = ctk.CTkFrame(self.blocks_wrap, fg_color=("gray95", "gray15"), corner_radius=6)
        ...
        ctk.CTkLabel(count_row, text="Nombre de blocs:", font=ctk.CTkFont(weight="bold")).pack(side="left")
        ...
        if button_parent is not None:
            ...
            PrimaryButton(button_container, text="Générer la séance", command=generate_callback).pack(
                fill="x", padx=16, pady=8
            )
        else:
            # Bouton dans le formulaire (comportement par défaut)
            PrimaryButton(button_container, text="Générer la séance", command=generate_callback).grid(
    ...
    def _rebuild_blocks_editor(self):
        ...
        for i in range(max(1, min(6, n))):
            ...
            block_container = ctk.CTkFrame(self.blocks_editor, fg_color=("gray92", "gray14"), corner_radius=8)
            ...
            ctk.CTkLabel(block_container, text="Format:").grid(row=1, column=0, sticky="w", padx=(12, 4), pady=(0, 8))
            fmt_var = ctk.StringVar(value=default_format)
            format_menu = ctk.CTkOptionMenu(block_container, variable=fmt_var, values=FORMATS, width=120)
            ...
            ctk.CTkLabel(duration_frame, text="min", font=ctk.CTkFont(size=11)).pack(side="left", padx=(2, 0))
    ...
    def _update_equipment_count(self):
        ...
        self.equip_count_label.configure(text=f"({count} sélectionné{'s' if count > 1 else ''})")
