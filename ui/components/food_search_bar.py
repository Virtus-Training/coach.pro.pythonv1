...
class FoodSearchBar(ctk.CTkFrame):
    ...
    def display_results(self, aliments: list, title: str = "Résultats") -> None:
        ...
        title_label = ctk.CTkLabel(
            self.results_frame, text=title, font=ctk.CTkFont(size=12, weight="bold")
        )
    ...
    def create_food_item(self, aliment) -> None:
        ...
        nutrition_text = (
            f"{aliment.kcal_100g:.0f} kcal | {aliment.proteines_100g:.1f}g prot"
        )
        ...
        if hasattr(aliment, "indice_healthy") and aliment.indice_healthy:
            score_color = (
                "green"
                if aliment.indice_healthy >= 7
                else "orange"
                if aliment.indice_healthy >= 4
                else "red"
            )
