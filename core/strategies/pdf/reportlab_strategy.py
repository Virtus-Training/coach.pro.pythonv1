...
class ReportLabPDFStrategy(BaseStrategy[PDFGenerationContext]):
    ...
    def _build_workout_document(
        self, story: List, data: Dict[str, Any], context: PDFGenerationContext
    ):
        ...
        if exercises:
            ...
            for i, exercise in enumerate(exercises, 1):
                ...
                if notes:
                    ...
                    story.append(
                        Paragraph(f"<b>Notes:</b> {notes}", self.styles["CustomBody"])
                    )
    ...
    def _build_nutrition_document(
        ...
        if client_info:
            ...
            client_data = [
                ...
                [
                    "Calories cibles:",
                    f"{client_info.get('target_calories', 'N/A')} kcal/jour",
                ],
    ...
    def _build_progress_document(
        ...
        if metrics:
            story.append(
                Paragraph("Métriques de Progression", self.styles["SectionHeader"])
            )
            story.append(Spacer(1, 10))
    ...
    def _add_progress_charts(self, story: List, charts_data: Dict[str, Any]):
        ...
        try:
            ...
            story.append(
                Paragraph(
                    f"Graphique non disponible: {str(e)}", self.styles["CustomBody"]
                )
            )
            story.append(Spacer(1, 20))
