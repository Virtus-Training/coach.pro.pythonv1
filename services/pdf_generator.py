...
def generate_nutrition_sheet_pdf(
    ...
    poids = (
        fiche_data.get("poids_kg_mesure")
        or getattr(client_data, "poids_kg", None)
        or ""
    )
    ...
    activite = getattr(client_data, "niveau_activite", "") or fiche_data.get(
        "niveau_activite", ""
    )
    ...
    c.setFillColor(colors.HexColor("#3C91E6"))
    c.rect(x, y - 0.25 * cm, 0.3 * cm, 0.3 * cm, stroke=0, fill=1)
    c.setFillColor(colors.black)
    c.drawString(x + 0.5 * cm, y, f"Protéines : {prot_g} g / {prot_cal} kcal")
    y -= 0.6 * cm
    c.setFillColor(colors.HexColor("#FFAD05"))
    c.rect(x, y - 0.25 * cm, 0.3 * cm, 0.3 * cm, stroke=0, fill=1)
    c.setFillColor(colors.black)
    c.drawString(x + 0.5 * cm, y, f"Glucides : {gluc_g} g / {gluc_cal} kcal")
    y -= 0.6 * cm
    c.setFillColor(colors.HexColor("#E4572E"))
    c.rect(x, y - 0.25 * cm, 0.3 * cm, 0.3 * cm, stroke=0, fill=1)
    c.setFillColor(colors.black)
    c.drawString(x + 0.5 * cm, y, f"Lipides : {lip_g} g / {lip_cal} kcal")
    y -= 0.9 * cm
    ...
    variation_kg = (
        round((delta * 30) / 7700.0, 1) if maintenance and objectif_kcal else 0.0
    )
    ...
    c.drawString(
        x + 0.5 * cm, block_y + block_h - 0.9 * cm, "Recommandations personnalisées"
    )

    conseils = [
        f"Hydratation : env. {int((poids or 0) * 35)} mL / jour"
        if poids
        else "Hydratation : viser ~2 L / jour",
        "Sommeil : 7 à 9h / nuit",
