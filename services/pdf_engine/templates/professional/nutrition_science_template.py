...
class NutritionScienceTemplate(BaseTemplate):
    ...
    def _build_analytics_dashboard(self) -> Table:
        ...
        protein_pct = (
            round((protein_g * 4 / total_calories) * 100, 1)
            if total_calories > 0
            else 0
        )
        carbs_pct = (
            round((carbs_g * 4 / total_calories) * 100, 1) if total_calories > 0 else 0
        )
        fat_pct = (
            round((fat_g * 9 / total_calories) * 100, 1) if total_calories > 0 else 0
        )
        ...
        analytics_data = [
            ...
            [
                ...
                f"TDEE {'+' if target_calories > tdee else ''}{target_calories - tdee:+d}",
            ],
            [
                "Protéines",
                f"{protein_g}g ({protein_pct}%)",
                f"{round(protein_g / nutrition_data.get('weight', 70), 1)}g/kg",
            ],
            ["Glucides", f"{carbs_g}g ({carbs_pct}%)", "45-65% calories totales"],
            ["Lipides", f"{fat_g}g ({fat_pct}%)", "20-35% calories totales"],
            [
                "Ratio P:C:L",
                f"{protein_pct}:{carbs_pct}:{fat_pct}",
                "Optimisation performance",
            ],
            [
                "Fibres Cibles",
                f"{nutrition_data.get('fiber_target', 25)}g",
                "14g/1000 kcal",
            ],
            [
                "Hydratation",
                f"{nutrition_data.get('water_target', 35)}ml/kg",
                "EFSA 2010",
            ],
        ]

        # Create combined layout
        dashboard_layout = [[analytics_data, macro_wheel]]
        ...
        analytics_table = Table(
            analytics_data, colWidths=[2 * inch, 1.5 * inch, 2 * inch]
        )
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 0),
                        (-1, 0),
                        HexColor(self.merged_config["colors"]["data_primary"]),
                    ),
    ...
    def _build_macro_science_analysis(self) -> Table:
        ...
        analysis_table = Table(
            analysis_data, colWidths=[1.2 * inch, 1.5 * inch, 1.8 * inch, 2 * inch]
        )
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 0),
                        (-1, 0),
                        HexColor(self.merged_config["colors"]["data_secondary"]),
                    ),
                    ("TEXTCOLOR", (0, 0), (-1, 0), HexColor("#ffffff")),
                    # Subheader
                    ("FONTNAME", (0, 1), (-1, 1), "Helvetica-Bold"),
    ...
    def _build_micronutrient_analysis(self) -> Table:
        ...
        micronutrients_data = [
            ...
            [
                "Vitamine D",
                f"{micro_data.get('vit_d', 15)}μg",
                "20-50μg",
                self._get_status(micro_data.get("vit_d", 15), 20, 50),
            ],
            [
                "B12",
                f"{micro_data.get('b12', 2.4)}μg",
                "2.4-100μg",
                self._get_status(micro_data.get("b12", 2.4), 2.4, 100),
            ],
            [
                "Fer",
                f"{micro_data.get('iron', 14)}mg",
                "8-18mg",
                self._get_status(micro_data.get("iron", 14), 8, 18),
            ],
            [
                "Magnésium",
                f"{micro_data.get('magnesium', 320)}mg",
                "400-420mg",
                self._get_status(micro_data.get("magnesium", 320), 400, 420),
            ],
            [
                "Zinc",
                f"{micro_data.get('zinc', 9)}mg",
                "8-11mg",
                self._get_status(micro_data.get("zinc", 9), 8, 11),
            ],
            [
                "Oméga-3",
                f"{micro_data.get('omega3', 1.2)}g",
                "2-3g",
                self._get_status(micro_data.get("omega3", 1.2), 2, 3),
            ],
            [
                "Créatine",
                f"{micro_data.get('creatine', 0)}g",
                "3-5g",
                self._get_status(micro_data.get("creatine", 0), 3, 5),
            ],
            [
                "Caféine",
                f"{micro_data.get('caffeine', 100)}mg",
                "3-6mg/kg",
                "Timing optimal",
            ],
        ]
        ...
        micro_table = Table(
            micronutrients_data,
            colWidths=[1.5 * inch, 1.2 * inch, 1.3 * inch, 2 * inch],
        )
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 0),
                        (-1, 0),
                        HexColor(self.merged_config["colors"]["chart_accent"]),
                    ),
                    ("TEXTCOLOR", (0, 0), (-1, 0), HexColor("#ffffff")),
                    # Subheader
                    ("FONTNAME", (0, 1), (-1, 1), "Helvetica-Bold"),
    ...
    def _build_metabolic_calculations(self) -> Table:
        ...
        calc_data = [
            ...
            [
                ...
                f"{metabolism.get('bmr', 1650)} kcal",
                "Mifflin-St Jeor",
            ],
            [
                "NEAT (Thermogenèse)",
                f"{metabolism.get('neat', 350)} kcal",
                "15-30% TDEE",
            ],
            [
                "TEF (Effet Thermique)",
                f"{metabolism.get('tef', 220)} kcal",
                "10% apport calorique",
            ],
            [
                "EAT (Exercice)",
                f"{metabolism.get('eat', 300)} kcal",
                "Cardio + Résistance",
            ],
            [
                "TDEE Total",
                f"{metabolism.get('tdee', 2520)} kcal",
                "BMR + NEAT + TEF + EAT",
            ],
            [
                "Déficit/Surplus Cible",
                f"{metabolism.get('target_deficit', -500)} kcal",
                "0.5-1kg/semaine",
            ],
            [
                "Protein Leverage",
                f"{metabolism.get('protein_leverage', 2.2)}g/kg",
                "Performance + Satiété",
            ],
            [
                "Leucine Threshold",
                f"{metabolism.get('leucine_threshold', 2.5)}g",
                "mTOR activation",
            ],
        ]
    ...
    def _build_meal_timing_analysis(self) -> Table:
        ...
        timing_analysis = [
            ...
            [
                "Petit-déjeuner",
                timing_data.get("breakfast_macros", "25:45:30"),
                "2-3h avant",
                "Glycémie stable (O'Neil et al.)",
            ],
            [
                "Collation Pré-WO",
                timing_data.get("preworkout_macros", "10:80:10"),
                "30-60min avant",
                "Performance glycolytique",
            ],
            [
                "Post-Workout",
                timing_data.get("postworkout_macros", "40:50:10"),
                "0-30min après",
                "Fenêtre anabolique",
            ],
            [
                "Déjeuner",
                timing_data.get("lunch_macros", "30:40:30"),
                "2-3h post-WO",
                "Récupération complète",
            ],
            [
                "Collation PM",
                timing_data.get("snack_macros", "20:20:60"),
                "Entre repas",
                "Hormones et satiété",
            ],
            [
                "Dîner",
                timing_data.get("dinner_macros", "35:25:40"),
                "3h avant coucher",
                "Récupération nocturne",
            ],
            [
                "Casein (optionnel)",
                timing_data.get("casein_macros", "90:5:5"),
                "Avant coucher",
        ...
        timing_table = Table(
            timing_analysis, colWidths=[1.5 * inch, 1.2 * inch, 1.3 * inch, 2 * inch]
        )
    ...
    def _build_scientific_recommendations(self) -> Table:
        ...
        rec_data = [
            ...
            [
                "Hydratation",
                recommendations.get(
                    "hydration", "35ml/kg + 500-750ml/h exercice (Position ACSM)"
                ),
            ],
            [
                "Timing Protéine",
                recommendations.get(
                    "protein_timing", "20-25g leucine riche dans 2h post-exercice"
                ),
            ],
            [
                "Supplémentation",
                recommendations.get(
                    "supplements", "Créatine 3g/j + Vitamine D3 2000UI si déficit"
                ),
            ],
            [
                "Récupération",
                recommendations.get(
                    "recovery", "Glucides 1-1.2g/kg dans 4h post-exercice"
                ),
            ],
            [
                "Performance",
                recommendations.get(
                    "performance", "Caféine 3-6mg/kg 30-60min pré-exercice"
                ),
            ],
            [
                "Composition Corporelle",
                recommendations.get(
                    "body_comp", "Déficit modéré 300-500kcal + entraînement résistance"
                ),
            ],
            [
                "Surveillance",
                recommendations.get(
                    "monitoring",
                    "Pesée hebdomadaire + photos + biomarqueurs trimestriels",
                ),
            ],
        ]
    ...
    def _build_biomarkers_correlation(self) -> Table:
        ...
        biomarkers_data = [
            ...
            [
                "HbA1c",
                f"{biomarkers.get('hba1c', 5.2)}%",
                "<5.5%",
                "Gestion glucides et timing",
            ],
            [
                "Lipides (HDL/LDL)",
                f"{biomarkers.get('hdl', 60)}/{biomarkers.get('ldl', 100)}",
                ">60/<100",
                "Qualité lipides (oméga-3)",
            ],
            [
                "CRP",
                f"{biomarkers.get('crp', 1.2)}mg/L",
                "<1.0mg/L",
                "Anti-inflammatoire (polyphénols)",
            ],
            [
                "Ferritine",
                f"{biomarkers.get('ferritin', 80)}μg/L",
                "30-150μg/L",
                "Absorption fer + vitamine C",
            ],
            [
                "25(OH)D",
                f"{biomarkers.get('vitamin_d', 25)}ng/mL",
                ">30ng/mL",
                "Supplémentation D3",
            ],
            [
                "B12",
                f"{biomarkers.get('b12_serum', 350)}pg/mL",
                ">300pg/mL",
                "Sources B12 ou supplément",
            ],
            [
                "Homocystéine",
                f"{biomarkers.get('homocysteine', 8)}μmol/L",
                "<10μmol/L",
                "Folates + B6/B12",
            ],
        ]
        ...
        biomarkers_table = Table(
            biomarkers_data, colWidths=[1.5 * inch, 1.2 * inch, 1.3 * inch, 2 * inch]
        )
    ...
    def _get_status(
        self, current: float, min_optimal: float, max_optimal: float
    ) -> str:
