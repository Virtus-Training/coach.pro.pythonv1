...
class NutritionTherapeuticTemplate(BaseTemplate):
    ...
    def _build_medical_header(self) -> Table:
        ...
        credentials = self.data.get("practitioner_credentials", "Spécialiste Nutrition Thérapeutique")
        ...
        header_data = [
            ...
            [f"N° ADELI: {self.data.get('adeli_number', 'N/A')}", f"Ordonnance: {self.data.get('prescription_ref', 'N/A')}"]
        ]
        ...
        header_table.setStyle(TableStyle([
            ...
            ('FONTSIZE', (0, 1), (0, 1), 10),
            ('FONTSIZE', (1, 1), (1, 1), 13),
            ('TEXTCOLOR', (1, 1), (1, 1), HexColor(self.merged_config["colors"]["medical"])),
            ...
            ('TEXTCOLOR', (0, 3), (-1, 3), HexColor(self.merged_config["colors"]["text_secondary"])),

            ('ALIGN', (0, 0), (0, -1), 'LEFT'),
            ('ALIGN', (1, 0), (1, -1), 'RIGHT'),
            ('VALIGN', (0, 0), (-1, -1), 'TOP'),
            ('BOX', (0, 0), (-1, -1), 1, HexColor(self.merged_config["colors"]["medical"])),
    ...
    def _build_patient_medical_info(self) -> Table:
        ...
        patient_data = [
            ...
            ["Nom/Prénom:", patient.get("name", "Non spécifié"), "Âge: " + str(patient.get("age", "N/A"))],
            ["Poids actuel:", f"{patient.get('current_weight', 'N/A')} kg", "Taille: " + f"{patient.get('height', 'N/A')} cm"],
            ["IMC:", f"{patient.get('bmi', 'N/A')} kg/m²", "Poids cible: " + f"{patient.get('target_weight', 'N/A')} kg"],
            ["Diagnostic principal:", patient.get("primary_diagnosis", "Non spécifié"), ""],
            ["Comorbidités:", patient.get("comorbidities", "Aucune"), ""],
            ["Médecin prescripteur:", patient.get("prescribing_physician", "Non spécifié"), ""],
            ["Traitement médicamenteux:", patient.get("current_medications", "Aucun"), ""],
            ["Antécédents nutritionnels:", patient.get("nutrition_history", "Non documentés"), ""]
        ]

        patient_table = Table(patient_data, colWidths=[2*inch, 2.5*inch, 1.5*inch])
        patient_table.setStyle(TableStyle([
            # Header
            ('SPAN', (0, 0), (-1, 0)),
            ('FONTNAME', (0, 0), (-1, 0), 'Times-Bold'),
            ('FONTSIZE', (0, 0), (-1, 0), 11),
            ('BACKGROUND', (0, 0), (-1, 0), HexColor(self.merged_config["colors"]["therapeutic"])),
            ...
            ('GRID', (0, 0), (-1, -1), 0.5, HexColor(self.merged_config["colors"]["therapeutic"])),
    ...
    def _build_therapeutic_assessment(self) -> Table:
        ...
        assessment_data = [
            ...
            ["Besoins énergétiques:", f"{assessment.get('energy_needs', 'Non calculé')} kcal/j"],
            ["Besoins protéiques:", f"{assessment.get('protein_needs', 'Non calculé')} g/kg/j"],
            ["Restrictions hydriques:", assessment.get("fluid_restrictions", "Aucune")],
            ["Textures alimentaires:", assessment.get("texture_modifications", "Normales")],
            ["Voie d'alimentation:", assessment.get("feeding_route", "Orale exclusive")],
            ["Dénutrition (MNA/NRS):", assessment.get("malnutrition_score", "Score non évalué")],
            ["Troubles déglutition:", assessment.get("swallowing_issues", "Non détectés")],
            ["Intolérances digestives:", assessment.get("digestive_issues", "Aucune rapportée")],
            ["Objectifs thérapeutiques:", assessment.get("therapeutic_goals", "À définir")]
        ]
        ...
        assessment_table.setStyle(TableStyle([
            ...
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
            ('VALIGN', (0, 0), (-1, -1), 'TOP'),
            ('GRID', (0, 0), (-1, -1), 0.5, HexColor(self.merged_config["colors"]["accent"])),
    ...
    def _build_restrictions_allergens(self) -> Table:
        ...
        restrictions_table = Table(restrictions_data, colWidths=[2.2*inch, 2.8*inch, 1*inch])
        restrictions_table.setStyle(TableStyle([
            # Header
            ('SPAN', (0, 0), (-1, 0)),
            ('FONTNAME', (0, 0), (-1, 0), 'Times-Bold'),
            ('FONTSIZE', (0, 0), (-1, 0), 11),
            ...
            ('TEXTCOLOR', (2, 2), (2, 2), HexColor(self.merged_config["colors"]["error"])),  # Allergies
            ('TEXTCOLOR', (2, 5), (2, 5), HexColor(self.merged_config["colors"]["error"])),  # Drug interactions
    ...
    def _build_therapeutic_nutrition_plan(self) -> Table:
        ...
        plan_data = [
            ...
            [
                "Petit-déjeuner",
                nutrition_plan.get("breakfast_composition", "À définir selon pathologie"),
            ...
            [
                ...
                nutrition_plan.get("supplement_notes", "Surveillance effets secondaires")
            ]
        ]

        plan_table = Table(plan_data, colWidths=[1.2*inch, 2*inch, 1.3*inch, 1.5*inch])
        plan_table.setStyle(TableStyle([
            # Header
            ('SPAN', (0, 0), (-1, 0)),
            ('FONTNAME', (0, 0), (-1, 0), 'Times-Bold'),
            ('FONTSIZE', (0, 0), (-1, 0), 11),
            ('BACKGROUND', (0, 0), (-1, 0), HexColor(self.merged_config["colors"]["medical"])),
            ('TEXTCOLOR', (0, 0), (-1, 0), HexColor("#ffffff")),
            ...
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
            ('VALIGN', (0, 0), (-1, -1), 'TOP'),
            ('GRID', (0, 0), (-1, -1), 0.5, HexColor(self.merged_config["colors"]["medical"])),
    ...
    def _build_monitoring_compliance(self) -> Table:
        ...
        monitoring_data = [
            ...
            ["📊 Poids corporel", monitoring.get("weight_frequency", "Hebdomadaire"), f"±{monitoring.get('weight_threshold', '2')}kg/mois"],
            ["🩸 Paramètres biologiques", monitoring.get("bio_frequency", "Selon prescription"), "Selon normes laboratoire"],
            ["🍽️ Apports alimentaires", monitoring.get("intake_frequency", "Quotidien"), "< 75% des recommandations"],
            ["💧 Bilan hydrique", monitoring.get("fluid_frequency", "Si applicable"), monitoring.get("fluid_limits", "Selon pathologie")],
            ["😌 Tolérance digestive", monitoring.get("tolerance_frequency", "Quotidien"), "Symptômes persistants >48h"],
            ["💊 Observance suppléments", monitoring.get("supplement_frequency", "Si prescrits"), "< 80% des prises"],
            ["📝 Tenue carnet alimentaire", monitoring.get("diary_frequency", "Première semaine"), "Manque >2 jours consécutifs"],
            ["👨‍⚕️ Consultation de suivi", monitoring.get("followup_frequency", "Selon protocole"), "Dégradation état général"]
        ]

        monitoring_table = Table(monitoring_data, colWidths=[2.2*inch, 1.8*inch, 2*inch])
        monitoring_table.setStyle(TableStyle([
            # Header
            ('SPAN', (0, 0), (-1, 0)),
            ('FONTNAME', (0, 0), (-1, 0), 'Times-Bold'),
            ('FONTSIZE', (0, 0), (-1, 0), 11),
            ('BACKGROUND', (0, 0), (-1, 0), HexColor(self.merged_config["colors"]["caution"])),
            ('TEXTCOLOR', (0, 0), (-1, 0), HexColor("#ffffff")),
            ...
            # Alert thresholds
            ('TEXTCOLOR', (2, 2), (2, -1), HexColor(self.merged_config["colors"]["error"])),
            ...
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
            ('VALIGN', (0, 0), (-1, -1), 'TOP'),
            ('GRID', (0, 0), (-1, -1), 0.5, HexColor(self.merged_config["colors"]["caution"])),
            ('TOPPADDING', (0, 0), (-1, -1), 4),
            ('BOTTOMPADDING', (0, 0), (-1, -1), 4)
        ]))
    ...
    def _build_substitutions_guide(self) -> Table:
        ...
        substitutions_data = [
            ...
            ["Aliment à Éviter", "Substitution Recommandée", "Justification Thérapeutique"],
            [
        ...
        substitutions_table = Table(substitutions_data, colWidths=[2*inch, 2*inch, 2*inch])
        substitutions_table.setStyle(TableStyle([
            # Header
            ('SPAN', (0, 0), (-1, 0)),
            ('FONTNAME', (0, 0), (-1, 0), 'Times-Bold'),
            ('FONTSIZE', (0, 0), (-1, 0), 11),
    ...
    def _build_medical_followup(self) -> Table:
        ...
        followup_data = [
            ...
            ["Prochaine consultation:", followup.get("next_consultation", "À programmer selon évolution")],
            ["Paramètres à surveiller:", followup.get("monitoring_parameters", "Selon protocole pathologie")],
            ["Critères de réussite:", followup.get("success_criteria", "À définir avec équipe médicale")],
            ["Critères d'alerte:", followup.get("alert_criteria", "Dégradation état nutritionnel")],
            ["Communication équipe:", followup.get("team_communication", "Transmission régulière au médecin")],
            ["Documentation requise:", followup.get("documentation", "Carnet alimentaire + bilan mensuel")],
            ["Éducation thérapeutique:", followup.get("education_plan", "Selon besoin patient/famille")],
            ["Contact urgence:", followup.get("emergency_contact", "Médecin prescripteur ou service")],
            ["Révision plan:", followup.get("plan_revision", "Selon évolution clinique")]
        ]
