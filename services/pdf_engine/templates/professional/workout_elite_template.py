...
class WorkoutEliteTemplate(BaseTemplate):
    ...
    def _build_client_info(self) -> Table:
        ...
        info_data = [
            [
                self._to_paragraph("<b>INFORMATIONS CLIENT</b>", "Header"),
                self._to_paragraph("", "Body"),
            ],
            [
                self._to_paragraph("Client:", "Body"),
                self._to_paragraph(client_name, "Body"),
            ],
            [
                self._to_paragraph("Coach:", "Body"),
                self._to_paragraph(coach_name, "Body"),
            ],
            [
                self._to_paragraph("Date:", "Body"),
                self._to_paragraph(session_date, "Body"),
            ],
            [
                self._to_paragraph("Durée:", "Body"),
                self._to_paragraph(duration, "Body"),
            ],
        ]
    ...
    def _build_exercise_table(self, exercises: List[Dict[str, Any]]) -> Table:
        ...
        exercise_table = Table(
            exercise_data, colWidths=[5 * cm, 1.5 * cm, 1.5 * cm, 1.5 * cm, 3 * cm]
        )
