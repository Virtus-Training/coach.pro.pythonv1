...
class SessionHomeTemplate(BaseTemplate):
            ...
            "equipment_needed": [
                {
                    "item": "Tapis de sol",
                    "essential": True,
                    "substitution": "Serviette épaisse",
                },
                {
                    "item": "Bouteilles d'eau (2x1L)",
                    "essential": False,
                    "substitution": "Sacs de riz",
                },
                {
                    "item": "Chaise stable",
                    "essential": True,
                    "substitution": "Bord de canapé",
                },
                {
                    "item": "Mur libre",
                    "essential": True,
                    "substitution": "Porte fermée",
                },
            ],
            "exercises": [
                {
    ...
    def _build_home_header(self) -> Table:
        ...
        header_data = [
            ...
            [
                Paragraph(
                    f"<b>👤 Participant:</b> {participant}<br/><b>📅 Date:</b> {session_date}",
                    ParagraphStyle(
                        "ParticipantInfo",
                ...
                Paragraph(
                    f"<b>⏱️ Durée totale:</b> {duration}<br/><b>📏 Espace requis:</b> {space_required}",
                    ParagraphStyle(
                        "SessionSpecs",
                        fontSize=self.fonts["body"][1],
                        textColor=self.colors["text"],
    ...
    def _build_setup_requirements(self) -> Table:
        ...
        setup_data = [
            [
                Paragraph(
                    "<b>🏠 PRÉPARATION DE L'ESPACE - CONFIGURATION OPTIMALE</b>",
                    ParagraphStyle(
                        "SetupTitle",
            ...
            [
                Paragraph(
                    f"<b>🏠 Pièce recommandée:</b> {room_type}<br/>"
                    f"<b>💡 Éclairage:</b> {lighting}<br/>"
                    f"<b>🌬️ Ventilation:</b> {ventilation}<br/>"
                    f"<b>🏢 Type de sol:</b> {floor_type}",
                    ParagraphStyle(
                        "SetupDetails",
                        fontSize=self.fonts["body"][1],
                        textColor=self.colors["text"],
            ...
            [
                Paragraph(
                    "<b>✅ Checklist de préparation:</b><br/>"
    ...
    def _build_equipment_checklist(self) -> Table:
        ...
        equipment_data = [
            [
                Paragraph(
                    "<b>🔧 ÉQUIPEMENT REQUIS - ALTERNATIVES INCLUSES</b>",
                    ParagraphStyle(
                        "EquipmentTitle",
                        fontSize=self.fonts["heading"][1],
                        textColor=colors.white,
    ...
    def _build_home_exercises(self) -> List[Flowable]:
        ...
        content.append(
            Paragraph(
                "<b>💪 PROGRAMME HOME TRAINING - ADAPTATIONS INCLUSES</b>",
                ParagraphStyle(
                    "ExerciseTitle",
                    fontSize=self.fonts["heading"][1],
                    textColor=self.colors["primary"],
        ...
        for i, exercise in enumerate(exercises):
            ...
            exercise_data = [
                ...
                [
                    Paragraph(
                        f"<b>🔧 Équipement:</b> {equipment}<br/><b>🎯 Installation:</b> {setup_notes}",
                        ParagraphStyle(
                            "ExerciseSetup",
                            fontSize=self.fonts["practical"][1],
                            textColor=self.colors["text"],
            ...
            if home_adaptations:
                exercise_data.append(
                    [
                        Paragraph(
                            f"<b>🏠 Adaptation maison:</b> {home_adaptations}",
                            ParagraphStyle(
                                "HomeAdaptations",
                                fontSize=self.fonts["caption"][1],
                                textColor=self.colors["efficiency"],
            ...
            exercise_table.setStyle(
                TableStyle(
                    [
                        ...
                        else None,  # Span safety if present
                        ("SPAN", (0, 3), (1, 3))
                        if len(exercise_data) > 3
                        else None,  # Span substitutions if present
                        ("SPAN", (0, 4), (1, 4))
                        if len(exercise_data) > 4
                        else None,  # Span adaptations if present
            ...
            table_style = [
                style for style in exercise_table._tablestyle._cmds if style is not None
            ]
    ...
    def _build_safety_section(self) -> Table:
        ...
        safety_data = [
            [
                Paragraph(
                    "<b>⚠️ CONSIGNES DE SÉCURITÉ À DOMICILE ⚠️</b>",
                    ParagraphStyle(
                        "SafetyTitle",
                        fontSize=self.fonts["heading"][1],
                        textColor=colors.white,
        ...
        safety_data.append(
            [
                Paragraph(
                    "<b>🚨 En cas de problème:</b> Arrêter immédiatement l'exercice et consulter un professionnel si nécessaire",
                    ParagraphStyle(
                        "Emergency",
                        fontSize=self.fonts["emphasis"][1],
                        textColor=self.colors["safety"],
    ...
    def _build_efficiency_tips(self) -> Table:
        ...
        tips_data = [
            [
                Paragraph(
                    "<b>🚀 CONSEILS D'EFFICACITÉ - OPTIMISATION HOME TRAINING</b>",
                    ParagraphStyle(
                        "TipsTitle",
                        fontSize=self.fonts["heading"][1],
                        textColor=colors.white,
        ...
        for i in range(max_tips):
            ...
            tips_data.append(
                [
                    Paragraph(
                        f"✓ {before_tip}" if before_tip else "",
                        ParagraphStyle(
                            "Tip",
                            fontSize=self.fonts["practical"][1],
                            textColor=self.colors["text"],
                    ...
                    Paragraph(
                        f"✓ {during_tip}" if during_tip else "",
                        ParagraphStyle(
                            "Tip",
                            fontSize=self.fonts["practical"][1],
                            textColor=self.colors["text"],
        ...
        tips_data.append(
            [
                Paragraph(
                    "🏠 <b>L'entraînement à domicile peut être aussi efficace qu'en salle !</b> 💪",
                    ParagraphStyle(
                        "Motivation",
                        fontSize=self.fonts["emphasis"][1],
                        textColor=self.colors["primary"],
