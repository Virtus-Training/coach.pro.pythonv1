...
class WorkoutMedicalTemplate(BaseTemplate):
    ...
    def _build_medical_header(self) -> Table:
        ...
        header_data = [
            ...
            [f"{practitioner}, {credentials}", "PRESCRIPTION D'EXERCICES THÉRAPEUTIQUES"],
        ...
        header_table.setStyle(TableStyle([
            ...
            ('FONTSIZE', (0, 1), (0, 1), 11),
            ('FONTSIZE', (1, 1), (1, 1), 14),
            ('TEXTCOLOR', (1, 1), (1, 1), HexColor(self.merged_config["colors"]["primary"])),
    ...
    def _build_patient_information(self) -> Table:
        ...
        patient_data = [
            ...
            ["Nom:", patient.get("name", "Non spécifié"), "Âge: " + str(patient.get("age", "N/A"))],
            ...
            ["Date début traitement:", patient.get("treatment_start", "Non spécifiée"), "Nombre de séances: " + str(patient.get("session_count", "N/A"))],
            ["Médecin prescripteur:", patient.get("prescribing_physician", "Non spécifié"), "Objectif thérapeutique: " + patient.get("therapeutic_goal", "Récupération fonctionnelle")]
        ]
    ...
    def _build_medical_assessment(self) -> Table:
        ...
        assessment_data = [
            ...
            ["Limitations fonctionnelles:", assessment.get("functional_limitations", "Voir évaluation initiale")],
            ["Contre-indications absolues:", assessment.get("absolute_contraindications", "Aucune identifiée")],
            ["Contre-indications relatives:", assessment.get("relative_contraindications", "Douleur aiguë > 7/10")],
            ["Précautions particulières:", assessment.get("special_precautions", "Éviter mouvements brusques")],
            ["Médications affectant l'exercice:", assessment.get("medications", "Aucune interférence connue")],
            ["Dernière évaluation:", assessment.get("last_evaluation", "Date non spécifiée")]
        ]
        ...
        assessment_table.setStyle(TableStyle([
            ...
            ('TEXTCOLOR', (0, 3), (-1, 3), HexColor(self.merged_config["colors"]["error"])),  # Absolute contraindications
            ('TEXTCOLOR', (0, 4), (-1, 4), HexColor(self.merged_config["colors"]["warning"])),  # Relative contraindications
    ...
    def _build_treatment_objectives(self) -> Table:
        ...
        objectives_data = [
            ...
            ["Objectif principal:", objectives.get("primary_objective", "Restauration fonction")],
            ["Objectifs secondaires:", objectives.get("secondary_objectives", "Réduction douleur, amélioration mobilité")],
            ["Critères de progression:", objectives.get("progression_criteria", "Échelle EVA, amplitude articulaire")],
            ["Durée estimée du traitement:", objectives.get("estimated_duration", "6-8 semaines")],
            ["Fréquence des séances:", objectives.get("session_frequency", "3x/semaine")],
            ["Réévaluation prévue:", objectives.get("reevaluation_date", "Dans 2 semaines")],
            ["Critères d'arrêt:", objectives.get("stop_criteria", "Douleur > 7/10, inflammation aiguë")]
        ]
    ...
    def _build_medical_exercise_blocks(self) -> List[Any]:
        ...
        for i, block in enumerate(exercise_blocks):
            ...
            if block.get("tracking_parameters"):
                tracking_table = self._build_phase_tracking(block["tracking_parameters"])
    ...
    def _build_medical_exercise_table(self, block: Dict[str, Any]) -> Table:
        ...
        exercise_headers = ["Exercice", "Prescription", "Paramètres", "Précautions", "Progression"]
        ...
        combined_data = phase_data + [["PRESCRIPTION DÉTAILLÉE", "", "", "", ""]] + exercise_data

        exercise_table = Table(combined_data, colWidths=[1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch])
        exercise_table.setStyle(TableStyle([
            # Phase info
            ('FONTNAME', (0, 0), (0, 2), 'Times-Bold'),
            ('BACKGROUND', (0, 0), (-1, 2), HexColor(self.merged_config["colors"]["surface"])),
            ('SPAN', (1, 0), (-1, 0)),
            ('SPAN', (1, 1), (-1, 1)),
            ('SPAN', (1, 2), (-1, 2)),
            ...
            ('BACKGROUND', (0, 3), (-1, 3), HexColor(self.merged_config["colors"]["medical"])),
            ('TEXTCOLOR', (0, 3), (-1, 3), HexColor("#ffffff")),

            # Exercise table headers
            ('FONTNAME', (0, 4), (-1, 4), 'Times-Bold'),
            ('BACKGROUND', (0, 4), (-1, 4), HexColor(self.merged_config["colors"]["surface"])),
            ('FONTSIZE', (0, 4), (-1, 4), 8),
    ...
    def _build_phase_tracking(self, tracking_params: Dict[str, Any]) -> Table:
        ...
        tracking_data = [
            ...
            ["Amplitude articulaire", tracking_params.get("rom_initial", "N/A"), tracking_params.get("rom_target", "ROM complet")],
            ["Force musculaire", tracking_params.get("strength_initial", "N/A"), tracking_params.get("strength_target", "5/5")],
            ["Échelle douleur (EVA)", tracking_params.get("pain_initial", "N/A"), tracking_params.get("pain_target", "< 3/10")],
            ["Fonction (questionnaire)", tracking_params.get("function_initial", "N/A"), tracking_params.get("function_target", "> 80%")]
        ]
    ...
    def _build_monitoring_section(self) -> Table:
        ...
        monitoring_data = [
            ...
            ["Fonction globale", "Toutes les 2 semaines", "Stagnation: révision objectifs"],
            ["Observance traitement", "Continue", "< 80%: renforcement motivation"],
            ["Effets indésirables", "Chaque séance", "Tout effet: documentation obligatoire"]
        ]

        monitoring_table = Table(monitoring_data, colWidths=[2*inch, 2*inch, 2*inch])
    ...
    def _build_medical_notes(self) -> Table:
        ...
        notes_data = [
            ...
            ["Observations générales:", notes.get("general_observations", "Patient motivé et coopératif")],
            ["Adaptations nécessaires:", notes.get("necessary_adaptations", "Aucune à ce stade")],
            ["Réponse au traitement:", notes.get("treatment_response", "Évolution favorable")],
            ["Recommandations:", notes.get("recommendations", "Poursuivre programme actuel")],
            ["Prochaine réévaluation:", notes.get("next_evaluation", "Dans 2 semaines")],
            ["Communication avec médecin:", notes.get("physician_communication", "Rapport envoyé")],
            ["Notes pour continuité soins:", notes.get("continuity_notes", "Maintenir motivation patient")]
        ]
    ...
    def get_data_schema(cls) -> Dict[str, Any]:
        ...
        return {
            ...
            "required": ["facility_name", "practitioner_name", "practitioner_credentials"]
        }