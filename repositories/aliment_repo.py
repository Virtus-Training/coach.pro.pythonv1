...
class AlimentRepository:
    ...
    def get_top_by_nutrition(self, 
                           metric: str = "proteines_100g", 
                           limit: int = 20,
                           exclude_categories: Optional[List[str]] = None) -> List[Aliment]:
    ...
    def get_complementary_foods(self, base_aliment_id: int, objectif_macros: Dict[str, float]) -> List[Tuple[Aliment, float]]:
        ...
        deficit_proteines = max(0, objectif_macros.get("proteines_g", 0) - base_aliment.proteines_100g)
        deficit_glucides = max(0, objectif_macros.get("glucides_g", 0) - base_aliment.glucides_100g)
        deficit_lipides = max(0, objectif_macros.get("lipides_g", 0) - base_aliment.lipides_100g)
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        with db_manager.get_connection() as conn:
            stats = {
                "total_aliments": conn.execute("SELECT COUNT(*) FROM aliments").fetchone()[0],
                ...
                "avg_kcal": conn.execute("SELECT AVG(kcal_100g) FROM aliments").fetchone()[0],
                "avg_proteines": conn.execute("SELECT AVG(proteines_100g) FROM aliments").fetchone()[0],
