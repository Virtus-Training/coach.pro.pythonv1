    ...
    class TestTemplate(BaseTemplate):
        def _build_content(self) -> List[Any]:
            ...
            title_style = ParagraphStyle(
                "TestTitle", fontSize=16, textColor=colors.black, alignment=TA_LEFT
            )

            content.append(
                Paragraph(
                    f"<b>Test PDF - {self.data.get('title', 'No title')}</b>",
                    title_style,
                )
            )
            ...
            body_style = ParagraphStyle(
                "TestBody", fontSize=12, textColor=colors.black, alignment=TA_LEFT
            )
